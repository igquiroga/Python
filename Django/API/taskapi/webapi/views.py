...
class homeView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        context = self.get_context_data(title='TASK API')
...
class TasksViewLoginUser(LoginView):
    ...
    def form_invalid(self, form):
        print(form.errors.as_data()['__all__'])
        return super(TasksViewLoginUser, self).form_invalid(form)
...
class AddTaskViewList(LoginRequiredMixin,TemplateView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        try:
            my_token = Token.objects.get(user_id=current_user.id)
            context = self.get_context_data(istoken=True,data=False)
        except:
            data = {'error' : 'No token avalaible'}
            context = self.get_context_data(istoken=False,data=data)
        return self.render_to_response(context)
    def post(self,request,*args,**kwargs):
        current_user = request.user
        my_token = Token.objects.get(user_id=current_user.id)
